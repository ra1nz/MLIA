...
def stoc_grad_ascent0(data_matrix, class_labels):
    ...
    for i in range(m):
        ...
        weights = weights + multiply(multiply(data_matrix[i], alpha), error)
    return weights
...
def stoc_grad_ascent1(data_matrix, class_labels, num_iter=150):
    ...
    for j in range(num_iter):
        data_index = [1 for i in range(m)]
        for i in range(m):
            alpha = 4 / (1.0 + j + i) + 0.0001
            rand_index = int(random.uniform(0, len(data_index)))
            h = sigmoid(sum(data_matrix[rand_index] * weights))
            error = class_labels[rand_index] - h
            weights = weights + multiply(multiply(data_matrix[rand_index], alpha), error)
